...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if hasattr(insect, 'real_queen') and insect.real_queen == True:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        #if either there is no Ant in the Bee's place or if there is an Ant, 
        #but its blocks_path attribute is False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place != hive:
            # if there is any bee in that place?
            if len(current_place.bees) != 0 and \
                i >= self.min_range and \
                i <= self.max_range:

                return random_or_none(current_place.bees)
            i += 1
            #consider the next place that is stored as the current place's entrance.
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        the current place.
        """
        # similar to the reduce_armor function in Insect
        self.armor -= amount
        if self.armor <=0:
...
class LongThrower(ThrowerAnt):
    ...
    implemented = True   
    min_range = 5 #update
    #ThrowerAnt.nearest_bee(self,hive), no need to call this
    #there are no self and hive attribute in this class anyway
...
# The WallAnt class
# Inheritance from the Ant class
class WallAnt(Ant):
    name = 'Wall'
    food_cost = 4
    #just update the armor
...
class HungryAnt(Ant):
    ...
    def __init__(self):
        ...
        # need to inheritance from Ant to get the armor
        Ant.__init__(self,armor = 1)
        self.digesting = 0

    def eat_bee(self, bee):
        #Only eat if there is a bee to eat
        if bee:
            bee.reduce_armor(bee.armor)
            # after eating, hungryant needs to wait 3 counts
...
class BodyguardAnt(Ant):
    ...
    #take in an ant argument, ant = None by default
    ...
    def action(self, colony):
        if self.ant!=None:
            # the ant it contained still do the action like normal
            # still produce food, or attack, or ...
...
def make_slow(action):
    ...
    def cha_dur (duration):
        def calling_this(colony):
            nonlocal duration
            duration-=1
            if duration >=0:
